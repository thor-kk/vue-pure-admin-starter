...
import { ReText } from '@/components/pure/ReText'
...
const { title, device, pureApp, getLogo, isCollapse, tooltipEffect, menuSelect, backTopMenu, toggleSideBar } = useNav()
...
function getSubMenuData() {
  ...
  useAppStoreHook().showDouble(childMenu.value.length > 1 || childMenu.value[0]?.meta?.showParent ? true : false)
...
<template>
  <div class="w-[290px]">
    <div :class="['double-left', 'w-[80px]', 'h-full', showLogo ? 'has-logo' : 'no-logo']">
      ...
      <el-scrollbar wrap-class="scrollbar-wrapper">
        <ul>
          ...
          >
            <div :class="['text-xl', isAllEmpty(menu?.meta.icon) ? 'mb-0' : 'mb-2']">
    ...
    >
      ...
      />
      <LaySidebarLeftCollapse :is-active="pureApp.sidebar.opened" @toggleClick="toggleSideBar" />
    </div>
  </div>
</template>
